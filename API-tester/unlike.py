"""
Let the user remove a name from the liked list.
The user must be logged in to remove a name from the liked list.
The user must provide a list of name ID(s) to remove from the liked list.
"""
...
async def unlike(
    ...
    name_ids: List[int] = Query(..., examples=1001), # type: ignore
    db: Connection = Depends(get_db),
    session_token: str = Cookie(None)
):

    """DELETE, request to remove a name from the liked list."""
